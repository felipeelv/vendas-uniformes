...
export default function CheckoutForm({
  ...
  const fieldClass = (field: string, validation: { valid: boolean; message?: string }) => {
    ...
    if (!touched[field] || !formData[field as keyof typeof formData]) {
      return `${base} bg-slate-50 border-slate-200 focus:ring-2 focus:ring-eleve-teal focus:border-eleve-teal`;
    }
    if (validation.valid) {
      return `${base} bg-eleve-teal/5 border-eleve-teal/40 focus:ring-2 focus:ring-eleve-teal focus:border-eleve-teal`;
    }
  ...
  const renderFieldFeedback = (field: string, validation: { valid: boolean; message?: string }) => {
    ...
    if (validation.valid) {
      return (
        <span className="flex items-center gap-1 text-[11px] font-medium text-eleve-teal-dark mt-1.5">
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-4">
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
              <h2 className="text-base font-bold text-slate-900 mb-5 flex items-center gap-2">
                <User className="w-4 h-4 text-eleve-teal" />
                Dados para o Pedido
              </h2>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400 font-medium px-1">
              <ShieldCheck className="w-4 h-4 text-eleve-teal shrink-0" />
            ...
            <button
              ...
              className={`lg:hidden w-full py-4 rounded-xl font-bold text-base text-white shadow-xl transition-all flex items-center justify-center gap-2 ${
                allValid
                  ? 'bg-eleve-teal hover:bg-eleve-teal-dark shadow-eleve-teal/20 hover:-translate-y-0.5 active:translate-y-0'
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 lg:sticky lg:top-24 shadow-sm">
              ...
              <div className="border-t border-slate-100 pt-4 space-y-2.5">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-eleve-teal-dark">Grátis na escola</span>
                </div>
              ...
              <button
                ...
                className={`hidden lg:flex w-full mt-5 py-4 rounded-xl font-bold text-base text-white transition-all items-center justify-center gap-2 ${
                  allValid
                    ? 'bg-eleve-teal hover:bg-eleve-teal-dark shadow-xl shadow-eleve-teal/20 hover:-translate-y-0.5 active:translate-y-0'
                    : 'bg-slate-300 cursor-not-allowed shadow-none'
                }`}
              >
                {submitting ? (
